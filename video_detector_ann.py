...
clf = load_model('new.model')    
files= 'VideoTest\VID_20190729_0751592 (1).mp4'
...
while (cap.isOpened()):
    ...
    #fgmask = fgbg.apply(frame,frame,0.01)
    ...
    for im_scaled in pyramid_gaussian(frame, downscale = downscale):
        ...
        for (x, y, im_window) in sliding_window(im_scaled, min_wdw_sz, step_size):
            ...
            fd = fd.reshape(1, -1)
            #start= time.time()
            pred = clf.predict_classes(fd)
            
            if pred == 1:
                print("Pred",pred)
                if clf.predict_proba(fd) > 0.5:
                    print("Clf",clf.predict_proba(fd))
                    detections.append((int(x * (downscale**scale)), int(y * (downscale**scale)), clf.predict_proba(fd), 
